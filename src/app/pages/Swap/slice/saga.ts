...
import { swappsFinanceActions as actions } from '.';
import { GetPoolProps, GetTokenProps, SwappsFinanceErrorType } from './types';
import {
  calculateDayAgg,
  calculateHourAgg,
  convertToMuTez,
...
export function* getTokens({ transformTokens }: GetTokenProps) {
  ...
  try {
    ...
    if (tokens?.length > 0) {
      ...
      yield put(actions.tokensError(SwappsFinanceErrorType.TOKEN_NOT_FOUND));
    }
  } catch (err: any) {
    yield put(actions.tokensError(SwappsFinanceErrorType.RESPONSE_ERROR));
  }
}
...
export function* getPools({ transformPools }: GetPoolProps) {
  ...
  try {
    ...
    if (pools?.length > 0) {
      ...
      yield put(actions.poolsError(SwappsFinanceErrorType.POOL_NOT_FOUND));
    }
  } catch (err: any) {
    yield put(actions.poolsError(SwappsFinanceErrorType.RESPONSE_ERROR));
  }
}
...
export function* getPoolMetrics({
  ...
  try {
    ...
    if (poolMetrics?.length > 0) {
      ...
      yield put(actions.poolsError(SwappsFinanceErrorType.METRICS_NOT_FOUND));
    }
  } catch (err: any) {
    yield put(actions.poolMetricsError(SwappsFinanceErrorType.RESPONSE_ERROR));
  }
}
 ...
 */
export function* swappsFinanceSaga() {
